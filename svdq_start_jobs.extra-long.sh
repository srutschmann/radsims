...
for r in $(seq -f "%03g" ${MINREP} ${MAXREP}); do
  ...
  if [[ ! -f ${r}_concat_ado_snps_reduced.svdq.tre ]]; then
    ...
    if grep -Pq "SBATCH -p ${QUEUE}$" ${r}_concat_ado_snps_reduced.svdq.sh; then
      ...
      if ! $dryrun; then
        if [[ "${QUEUE}" == "fatnode" ]]; then
          echo "sbatch -c 24 -p thin-shared --qos=extra-long ${r}_concat_ado_snps_reduced.svdq.sh"
        elif [[ "${QUEUE}" == "thin-shared" ]]; then
          echo "sbatch -c 20 -p shared --qos=shared-xlong ${r}_concat_ado_snps_reduced.svdq.sh"
        fi
      fi
    fi
  fi
